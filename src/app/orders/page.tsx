        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {orders.map((order) => {
              ...
              const totalAfterDiscount = order.totalAfterDiscount ?? order.total;
              const hasCoupon = Boolean(order.couponCode) || discount > 0;
              ...
              return (
                <div key={order.id} className="bg-white rounded-3xl shadow-xl p-6 border border-[#efe7da]">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                    ...
                    {hasCoupon && (
                      <div>
                        <p className="text-sm text-gray-500">{t.orders.coupon}</p>
                        <p className="font-semibold text-gray-900">
                          {order.couponCode || "-"}
                        </p>
                      </div>
                    ...
                    {discount > 0 && (
                      <div>
                        <p className="text-sm text-gray-500">{t.cart.discount}</p>
                        <p className="font-semibold text-[#7a5a1f]">-${discount.toFixed(2)}</p>
                      </div>
                    )}
                    <div>
                      <p className="text-sm text-gray-500">{t.cart.total}</p>
                      <p className="font-semibold text-gray-900">${totalAfterDiscount.toFixed(2)}</p>
                    </div>
                  </div>
